---
title: "Haplotypes in the Chrom2 Spike Region, Alewife Only"
author: "Hana Moyle"
...
write_tsv(new_names, "data/alewife_subset/chr2_peak/intermediates/new_names_for_vcf.txt",
          col_names = FALSE)
```
...
bcftools reheader -s intermediates/new_names_for_vcf.txt vcf/noblues.vcf.gz > intermediates/noblues_renames.vcf.gz
## the filter the sites
conda acitvate vcftools # activate vcftools environment
vcftools --gzvcf intermediates/noblues_renames.vcf.gz --min-alleles 2 --max-alleles 2 --max-missing 0.4 --maf 0.05 --out intermediates/noblues_filtered --recode
## attach the ancestral alleles from American Shad
echo '##INFO=<ID=AA,Number=1,Type=String,Description="Ancestral allele">' > aa.hdr
conda deactivate
bcftools +fill-from-fasta intermediates/noblues_filtered.recode.vcf -- -c AA -f genome/GCF_018492685.1/GCF_018492685.1_fAloSap1.pri_genomic.fna.gz -h aa.hdr > intermediates/noblues_with_anc.vcf # I downloaded the full A. Shad genome from UCSC Genome Browser
```
...
```{r}
rfi <- read.vcfR("data/alewife_subset/chr2_peak/intermediates/noblues_with_anc.vcf")
...
bcftools reheader -s intermediates/new_names_for_vcf.txt phased/noblues.vcf.gz > phased/noblues.vcf.gz
tabix -f phased/noblues.vcf.gz
bcftools +fill-from-fasta phased/noblues.vcf.gz -- -c AA -f genome/GCF_018492685.1/GCF_018492685.1_fAloSap1.pri_genomic.fna.gz -h aa.hdr > phased/noblues_with_anc.vcf
```


```{r phased_data}
haps <- ecaRbioinf::vcf_haplos2tidy("data/alewife_subset/chr2_peak/phased/noblues_with_anc.vcf", 
                                    Anc = "Shad") # read in the phased vcf
...
bcftools view -s AP028666 vcf/noblues.vcf.gz > intermediates/one-sample-chr2-spike-ish-region.vcf.gz # i picked the highest coverage indiv
conda activate bedtools
bedtools intersect -a intermediates/one-sample-chr2-spike-ish-region.vcf.gz -b genome/GCF_018492685.1/genomic.gff -wb > intermediates/variant-annotations.tsv # intersect them
```

```{r}
snp_anno <- read_tsv("data/alewife_subset/chr2_peak/intermediates/variant-annotations.tsv", 
                     col_names = c(
                       "CHROM", 
                       "POS", 
                       "dot1", 
...
samtools faidx genome/GCF_018492685.1/GCF_018492685.1_fAloSap1.pri_genomic.fna.gz NC_055958.1:16600000-17200000 > intermediates/chr2-spike-ish-seq-shad.fna # index the section of the shad reference genome we are using
gzip -f intermediates/chr2-spike-ish-seq-shad.fna # zip up the baby
```

```{r, echo = FALSE}
dna_tibble <- read_lines("data/alewife_subset/chr2_peak/intermediates/chr2-spike-ish-seq-shad.fna.gz")[-1] %>%
  paste(., 
        collapse = "") %>%
  strsplit(., 
           "") %>%
...
xmids <- data.frame(midx = c(-10, -25), 
                    column = c("WATER_NAME", "grouping_v3"))

col_labs <- tibble(label = c("Population", "Managing Unit")) %>%
    mutate(
      xpos = xmids$midx,
...
```{r, eval = FALSE}
ggsave(plot, 
       filename = "figures/haplotypes/chr2_peak/haplo-raster-alleles-no-labs-alewife-only-test.png", 
       width = 49, 
       height = 30)
