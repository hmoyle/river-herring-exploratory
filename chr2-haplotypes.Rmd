...
write_tsv(new_names, "data/chr2_peak/phased/intermediates/new_names_for_vcf.txt",
          col_names = FALSE)
```
...
```{r}
rfi <- read.vcfR("data/chr2_peak/phased/intermediates/almosst-ready.vcf")
...
xmids <- data.frame(columns = c("WATER_NAME", 
                                "grouping_v3"), 
                    midx = c(-20, -50))

plot2 <- extra_labels(g = plot, 
                      D = plist$D, 
                      h_ord = h_ord_group_pop_middy, 
                      xmids = xmids, 
                      ymids = plist$ar_y_midpoints, 
                      row_just = 0.5, 
                      pop_size = 8.0, 
                      group_size = 10.0) +
  expand_limits(x = -100, y = 150)

ggsave(plot, 
       filename = "figures/haplotypes/haplo-raster-alleles-no-labs.png", 
       width = 49, 
       height = 30)
