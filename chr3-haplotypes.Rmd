---
title: "Haplotypes in the Chrom 3 Spike Region"
author: "Hana Moyle"
...
```{r}
rfi <- read.vcfR("data/chr3_peak/phased/intermediates/almost-ready.vcf")
...
```{r phased_data}
haps <- ecaRbioinf::vcf_haplos2tidy("data/chr3_peak/phased/namaglfl-phased-with-anc.vcf", Anc = "Shad") # read in the phased vcf
...
```{r}
snp_anno <- read_tsv("data/chr3_peak/phased/intermediates/variant-annotations.tsv", 
                     col_names = c(
                       "CHROM", 
                       "POS", 
                       "dot1", 
...
```{r, echo = FALSE}
dna_tibble <- read_lines("data/chr3_peak/phased/intermediates/chr3-spike-ish-seq-shad.fna.gz")[-1] %>%
  paste(., 
        collapse = "") %>%
  strsplit(., 
           "") %>%
...
ggsave(plot, 
       filename = "figures/haplotypes/chr3_peak/haplo-raster-alleles-no-labs.png", 
       width = 49, 
       height = 30)
