---
title: "Haplotypes in the Chrom 19 Spike Region"
author: "Hana Moyle"
subtitle: "Last Updated: 2024-07-19"
...
write_tsv(new_names, "data/chr19_peak/phased/intermediates/new_names_for_vcf.txt",
          col_names = FALSE)
```
...
bcftools reheader -s intermediates/new_names_for_vcf.txt ../vcf/namaglfl.vcf.gz > intermediates/for-depths.vcf.gz
...
bcftools +fill-from-fasta intermediates/namaglfl-chr19-filtered.recode.vcf -- -c AA -f data/GCF_018492685.1/GCF_018492685.1_fAloSap1.pri_genomic.fna.gz -h aa.hdr > intermediates/almost-ready.vcf # I downloaded the full A. Shad genome from UCSC Genome Browser
```
...
```{r}
rfi <- read.vcfR("data/chr19_peak/phased/intermediates/almost-ready.vcf")
...
bcftools reheader -s intermediates/new_names_for_vcf.txt namaglfl.vcf.gz > namaglfl-phased.vcf.gz # and renaming the samples + getting the correct file name
tabix -f namaglfl-phased.vcf.gz # now I'm using the phased vcf from beagle_regions/../phased/ 
bcftools +fill-from-fasta namaglfl-phased.vcf.gz -- -c AA -f data/GCF_018492685.1/GCF_018492685.1_fAloSap1.pri_genomic.fna.gz -h aa.hdr > namaglfl-phased-with-anc.vcf # and again attaching the ancestral states

```


```{r phased_data}
haps <- ecaRbioinf::vcf_haplos2tidy("data/chr19_peak/phased/namaglfl-phased-with-anc.vcf", Anc = "Shad") # read in the phased vcf

samples <- sample %>%
  select(newname, STATE_F, WATERSHED, WATER_NAME, SPECIES, grouping_v3) %>%
  filter(grouping_v3 == "MIDA"| 
         grouping_v3 == "NATLA"| 
         grouping_v3 == "GRTL"| 
         grouping_v3 == "FINL") # select important field from the metadata
...
bcftools view -s AP028690 ../vcf/namaglfl.vcf.gz > intermediates/one-sample-vcf-chr19-spike-ish.vcf.gz # i picked one of the mid-atlantic anadromous alewife ones
mamba create -c conda-forge -c bioconda -n bedtools bedtools # didn't have bedtools installed, so I installed it
conda activate bedtools
bedtools intersect -a intermediates/one-sample-vcf-chr19-spike-ish.vcf.gz -b data/GCF_018492685.1/genomic.gff -wb > intermediates/variant-annotations.tsv # intersect them
```

```{r}
snp_anno <- read_tsv("data/chr19_peak/phased/intermediates/variant-annotations.tsv", 
                     col_names = c(
                       "CHROM", 
                       "POS", 
                       "dot1", 
...
samtools faidx data/GCF_018492685.1/GCF_018492685.1_fAloSap1.pri_genomic.fna.gz NC_055975.1:2200000-8500000 > intermediates/chr19-spike-ish-seq-shad.fna.gz
```

```{r, echo = FALSE}
dna_tibble <- read_lines("data/chr19_peak/phased/intermediates/chr19-spike-ish-seq-shad.fna.gz")[-1] %>%
  paste(., 
        collapse = "") %>%
  strsplit(., 
           "") %>%
...
annotation_columns <- chr19_spike_sorts %>%
  mutate(hnames = haplo_name) %>%
...
plist <- prep_elements(plo = 2200000, 
                       phi = 8500000, 
                       pby = 500000, 
                       format = "%.2f", 
                       DSET = big_haps2)
...
tmp <- plist$D %>%
  ...
  mutate(hnfact = factor(haplo_name,
                         levels = rev(h_ord_group_pop_middy))) %>%
  arrange(hnfact) %>%
  group_by(hnfact) %>%
  slice(1) %>%
  ungroup() %>%
...
ggsave(plot, 
       filename = "figures/haplotypes/chr19_peak/haplo-raster-alleles-no-labs.png", 
       width = 49, 
       height = 30)
